...
import { LucideAngularModule, TrendingUp, AlertTriangle, DollarSign, BarChart3, ShieldCheck, Info, HelpCircle, ArrowUpRight, ArrowDownRight, Droplets, Lightbulb, BarChart2, PieChart, Globe } from 'lucide-angular';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule, AdelaideTimePipe],
...
export class ProfitRiskComponent {
  ...
  getSliderBackground() {
    ...
    return `linear-gradient(to right, #3b82f6 0%, #3b82f6 ${val}%, #e2e8f0 ${val}%, #e2e8f0 100%)`;
  }

  getBankruptcyAdjustedRevenue(cropName: string): number {
    const crop = this.cropMetrics().find(c => c.name === cropName);
    if (!crop) return 0;
    // If showBankruptcyImpact is true, simulate a drop in revenue (e.g. 50%)
    if (this.showBankruptcyImpact()) {
      return crop.revenuePerHa * 0.5;
    }
    return crop.revenuePerHa;
  }

  getZeroLinePosition(): number {
    const margins = this.cropMetrics().map(c => c.netMarginPerHa);
  ...
  getHoveredValue(metrics: any): number {
    if (!metrics || !this.hoveredSegment()) return 0;
    const seg = this.hoveredSegment();

    // Mapping segment names to 'metrics' properties
    // Assuming segment names match broadly with chart sections
    if (seg === 'Costs' || seg === 'Inputs') return metrics.costs;
    if (seg === 'Revenue') return metrics.revenue;
    if (seg === 'Capital') return metrics.capital;
    if (seg === 'Margin') return metrics.margin;
  ...
  getHoveredPercentage(metrics: any): number {
    ...
    if (metrics.isCritical) {
      if (seg === 'Costs') return metrics.costPercentage;
      if (seg === 'Revenue') return metrics.revenuePercentage;
    } else {
      if (seg === 'Inputs') return metrics.inputPct;
      if (seg === 'Capital') return metrics.capitalPct;
      if (seg === 'Margin') return metrics.marginPct;
    }
    return 0;
  }
}
